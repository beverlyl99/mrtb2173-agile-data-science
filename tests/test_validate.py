...
def make_clean_df():
    """Helper: create a small clean test DataFrame"""
    return pd.DataFrame({
...
def test_no_missing_values():
    df = make_clean_df()
    assert df.isnull().sum().sum() == 0, "Clean df should have no missing values"

def test_no_duplicates():
    df = make_clean_df()
    assert df.duplicated().sum() == 0, "Clean df should have no duplicates"

def test_no_negative_quantity():
    df = make_clean_df()
    assert (df['Quantity'] < 0).sum() == 0, "Clean df should have no negative quantities"
...
def test_detects_missing_customer_id():
    ...
    assert df['Customer ID'].isnull().sum() == 1, "Should detect 1 missing Customer ID"

def test_detects_negative_quantity():
    df = make_clean_df()
    df.loc[1, 'Quantity'] = -3.0
    assert (df['Quantity'] < 0).sum() == 1, "Should detect 1 negative quantity"
